...
function renderCalendar(state){
  ...
  let selRegions=[], selMarkets=[], selType='';

  function getActsFiltered(market, monthIdx){
    return acts.filter(a=>{
      if(a.market!==market) return false;
      if(selType && a.activityType!==selType) return false;
  ...
  function redrawAll(){
    ...
    const typeLabel = selType ? ' \xb7 '+selType : '';
  ...
  const typeOpts = allTypes.map(t=>'<option value="'+t+'">'+t+'</option>').join('');
  ...
  document.getElementById('view-area').innerHTML =
    ...
    '<div class="cal-filter-bar mb20">'+
      '<label class="flt-label">Activity Type:</label>'+
      '<select id="cal-type-sel" class="flt-select" style="min-width:200px"><option value="">All Types</option>'+typeOpts+'</select>'+
  ...
  document.getElementById('cal-type-sel')?.addEventListener('change', e=>{
  ...
  window.calReset = ()=>{
    selType=''; selRegions.length=0; selMarkets.length=0;
    clearMS('ms-calreg','Region'); clearMS('ms-calmkt','Market');
    const s=document.getElementById('cal-type-sel'); if(s) s.value='';
    redrawAll();
  };
  ...
  window.calMonth = (mkt, mo, idx)=>{
    ...
    if(detRow.classList.contains('hidden') || detIn.dataset.key !== mkt+'-'+mo){
      ...
      if(monthActs.length){
        ...
        const typeNote = selType ? ' (' + selType + ')' : '';
  ...
  window.calExpand = (mkt)=>{
    ...
    const mActs = acts.filter(a=>a.market===mkt && (!selType||a.activityType===selType))

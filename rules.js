...
function runRules(baseline26,review27){
  ...
  // 1.1
  A27.forEach(a=>{const k=`${a.market}||${(a.activityName||'').toLowerCase().trim()}`;const prev=map26[k]||0;if(prev>0&&a.cashflow>prev){const pct=((a.cashflow-prev)/prev)*100,abs=a.cashflow-prev;if(pct>THRESH.INC_PCT&&abs>THRESH.INC_AED)violations.push(V('1.1',a,`${fmtAED(prev)} (2026) → ${fmtAED(a.cashflow)} (2027). +${fmtAED(abs)} (+${pct.toFixed(1)}%)`));}});
  // 1.2 Nov+Dec >20%
  ...
  // 1.7
  A27.filter(isExistJMP).forEach(a=>{if(a.cashflow===0&&a.locked==='Locked')violations.push(V('1.7',a,'Locked Existing JMP cashflow=0. Contract value may be missing.'));});
  // 2.2
...
function compareYears(b26,r27){const A26=b26.activities||[],A27=r27.activities||[];const m26={},m27={};A26.forEach(a=>{m26[`${a.market}||${(a.activityName||'').toLowerCase().trim()}`]=a;});A27.forEach(a=>{m27[`${a.market}||${(a.activityName||'').toLowerCase().trim()}`]=a;});const added=A27.filter(a=>!m26[`${a.market}||${(a.activityName||'').toLowerCase().trim()}`]);const removed=A26.filter(a=>!m27[`${a.market}||${(a.activityName||'').toLowerCase().trim()}`]);const changed=[];Object.entries(m27).forEach(([k,a27])=>{const a26=m26[k];if(!a26)return;const ch=[];if(Math.abs(a27.cashflow-a26.cashflow)>1000)ch.push({field:'Cashflow',from:a26.cashflow,to:a27.cashflow,diff:a27.cashflow-a26.cashflow});if(a27.priority!==a26.priority&&a27.priority&&a26.priority)ch.push({field:'Priority',from:a26.priority,to:a27.priority,diff:0});if(a27.activityType!==a26.activityType)ch.push({field:'Type',from:a26.activityType,to:a27.activityType,diff:0});if(a27.locked!==a26.locked)ch.push({field:'Lock',from:a26.locked,to:a27.locked,diff:0});if(ch.length)changed.push({a27,a26,changes:ch});});return{added,removed,changed};}
